...
export async function POST(
  ...
  try {
    ...
    const safeSource = geojson_source && VALID_SOURCES.has(geojson_source) ? geojson_source : null
    const safeAppType = application_type && VALID_APP_TYPES.has(application_type) ? application_type : null
    ...
    if (error) {
      console.error('[POST /api/trials/[id]/applications] DB error details:', {
        code: error.code,
        message: error.message,
        details: error.details,
        hint: error.hint,
      })
      return safeErrorResponse(error, 'POST /api/trials/[id]/applications')
    }
    return NextResponse.json(data, { status: 201 })
  } catch (err) {
    return safeErrorResponse(err, 'POST /api/trials/[id]/applications (unhandled)')
  }
}
