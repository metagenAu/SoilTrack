-- Clear raw_rows JSONB from raw_uploads after data has been successfully loaded.
-- This prevents the raw_uploads table from becoming the primary driver of database
-- bloat, since raw_rows stores the full contents of uploaded CSV/Excel files.

-- 1. Null out raw_rows for all previously loaded uploads
UPDATE raw_uploads
SET raw_rows = '[]'::jsonb
WHERE status = 'loaded';

-- 2. Update the load_and_track function to clear raw_rows after successful load
...
BEGIN
  ...
  -- Mark raw_upload as loaded and clear raw_rows to reclaim storage
  IF p_raw_upload_id IS NOT NULL THEN
    UPDATE raw_uploads
    SET status = 'loaded', records_loaded = v_count, raw_rows = '[]'::jsonb
