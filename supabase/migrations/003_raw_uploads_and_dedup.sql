-- Raw uploads staging table: stores untransformed data before parsing
-- Decouples file ingestion from data transformation, so data in unexpected
-- formats doesn't get lost — it sits here waiting to be mapped.
CREATE TABLE raw_uploads (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  trial_id    TEXT REFERENCES trials(id) ON DELETE CASCADE,
  filename    TEXT NOT NULL,
  file_type   TEXT,                  -- detected or user-specified classification
  raw_rows    JSONB NOT NULL,        -- array of parsed rows, exactly as read from CSV/Excel
  headers     TEXT[],                -- original column headers in file order
  column_map  JSONB,                 -- mapping used for transformation (null = pending review)
  unmapped_columns TEXT[],           -- columns that could not be auto-matched
  status      TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'mapped', 'loaded', 'error')),
  error_detail TEXT,
  records_loaded INT DEFAULT 0,
  created_at  TIMESTAMPTZ DEFAULT now()
);

CREATE INDEX idx_raw_uploads_trial ON raw_uploads(trial_id);
CREATE INDEX idx_raw_uploads_status ON raw_uploads(status);

-- Enable RLS
ALTER TABLE raw_uploads ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Authenticated users full access" ON raw_uploads
  FOR ALL USING (auth.role() = 'authenticated');

-- Deduplication: add unique constraints on natural keys for each data table.
-- These use COALESCE to handle nulls consistently.
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_plot_data_natural
  ON plot_data (trial_id, COALESCE(plot, ''), COALESCE(trt_number::text, ''), COALESCE(rep::text, ''));
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_sample_metadata_natural
  ON sample_metadata (trial_id, COALESCE(assay_type, ''), COALESCE(sample_no, ''), COALESCE(metric, ''));

-- Transaction helper: upsert data rows + update tracking tables atomically.
-- Called from app code via supabase.rpc('load_and_track', { ... })
...
BEGIN
  ...
  -- Each table has a different conflict target.
