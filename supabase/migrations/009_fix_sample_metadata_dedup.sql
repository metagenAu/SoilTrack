-- Fix: sample_metadata natural key was missing `date`.
-- sample_no maps to a treatment/block, and the same sample point can have
-- multiple timepoints. Without `date` in the key, uploading a second timepoint
...
-- Update load_and_track RPC to match the new natural key
...
BEGIN
  ...
  ELSIF p_table_name = 'soil_chemistry' THEN
    ...
    SELECT
      ...
      r->'raw_data'
    FROM jsonb_array_elements(p_rows) AS r
    ON CONFLICT (trial_id, COALESCE(sample_no, '__null__'), COALESCE(date, '1900-01-01'::date), COALESCE(metric, '__null__'))
    DO UPDATE SET
  ...
  ELSIF p_table_name = 'tissue_chemistry' THEN
    ...
    SELECT
      ...
      r->'raw_data'
    FROM jsonb_array_elements(p_rows) AS r
    ON CONFLICT (trial_id, COALESCE(sample_no, '__null__'), COALESCE(date, '1900-01-01'::date), COALESCE(tissue_type, '__null__'), COALESCE(metric, '__null__'))
    DO UPDATE SET
      value = EXCLUDED.value,
      unit = EXCLUDED.unit,
      raw_data = EXCLUDED.raw_data;
  ...
  ELSIF p_table_name = 'sample_metadata' THEN
    ...
    SELECT
      ...
      r->'raw_data'
    FROM jsonb_array_elements(p_rows) AS r
    ON CONFLICT (trial_id, COALESCE(assay_type, '__null__'), COALESCE(sample_no, '__null__'), COALESCE(date, '1900-01-01'::date), COALESCE(metric, '__null__'))
    DO UPDATE SET
      block = EXCLUDED.block,
